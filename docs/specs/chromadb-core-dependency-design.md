...
`RAG.__init__`（`src/ragline/api/rag.py:155`）无条件构造
...
chroma = ["chromadb>=0.5"]
all = ["ragline[server,bge,chroma,pdf]"]
```

裸装 `pip install ragline`（不带任何 extra）缺 chromadb → 即使
lazy-optional-imports（问题 2）修复后 `import ragline` 不崩，
...
裸装 `pip install ragline` 即可正常 `RAG()`。
...
| `uv.lock` | `uv lock` 机械重生成（chromadb 在 ragline 的依赖归类从可选转主依赖；包版本不变，已在锁内） |
| `README.md` | 修正 `pip install "ragline[chroma]"` 措辞：chromadb 现为默认安装，`[chroma]` extra 保留仅为向后兼容（no-op） |

### 3.2 关键设计决策

- **chroma extra 保留为空**：`chroma = []` 而非删除。已有用户/脚本/CI
  的 `pip install 'ragline[chroma]'` 仍合法、不报错，只是不再额外装包。
  零破坏性（用户已确认选此方案）。
...
- 验证 = 全量测试套件绿 + `uv lock --check` 一致 + 隔离环境裸装
  （无 extra）确认 chromadb 随主依赖装上、`import ragline` + 构造
  `RAG()`（不触发网络的最小构造）可用
...
- [ ] `uv lock --check` 一致；`uv.lock` 中 ragline 的 requires-dist 反映 chromadb 为主依赖
- [ ] 隔离环境 `pip install <wheel>`（无 extra）后 chromadb 已安装、
      `import ragline` 成功
- [ ] `pip install 'ragline[chroma]'` 不报 "no extra named chroma"
